...
//				request.getRequestDispatcher("vendoraddservice.jsp").forward(request, response);
			...
			if (MyDbImpl.vendorreg(v) == 1) {
				...
				out.println("<html><body><b>Something went wrong " + "<br><a href='vendorreg.html'>Please Click here to reg again..</a></b></body></html>");
//				request.getRequestDispatcher("HomeService1stPage.html").forward(request, response);
			}
...
//				out.println("<html><body><b>Successfully Inserted after login" + "</b></body></html>");
				request.setAttribute("vid", l.getVendorid());
				request.getRequestDispatcher("vendorafterlogin.jsp").forward(request, response);
			} else {
				request.getRequestDispatcher("Vendorupdatingdetails").forward(request, response);
			}
		} else if (submitType.equals("Update Details")) {
...
//					request.getRequestDispatcher("HomeService1stPage.html").forward(request, response);
				...
				if (MyDbImpl.vendorservice(vendorid, phone, avltimefrom, avltimeto, loc, servicetype1,categories) == 1) {
...
//					request.getRequestDispatcher("Vendorupdatingdetails").forward(request, response);
			...
			String vendorid = l.getVendorid();
			String phone = request.getParameter("telnum2");
			String avltimefrom = request.getParameter("avltimefrom2");
			String avltimeto = request.getParameter("avltimeto2");
			String loc = request.getParameter("location2");
...
//				request.getRequestDispatcher("Vendorupdatingdetails").forward(request, response);
		...
		String vendorid = l.getVendorid();
		String servicetype1 = request.getParameter("servicetype2");
		String categories = request.getParameter("category3");
		String categories1 = request.getParameter("category21");
		if(categories=="") {
			categories=categories1;
		}
		System.out.println("id"+vendorid+"st:"+servicetype1+"cat"+categories);
